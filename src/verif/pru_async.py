...
def replicate_val(val, hw_details):
  ...
  """
  assert (val >> hw_details.POSIT_L) == 0, "0x{:0x} { }".format(val, hw_details.POSIT_L)
  concat_val = 0
  for i in range(32//hw_details.POSIT_L):
...
#  last_global_barrier = 2
  ...
  log.info(f'last_global_barrier: {last_global_barrier}')
  ...
  print(op_count, instr_count, ld_count, st_count)
...
def path_prefix(global_var, config_obj):
  ...
  prefix += '_' + str(hw_details.POSIT_L) + 'b'
