...
def main(global_var, net, graph, graph_nx, node_w, config_obj, final_output_nodes, verbose= True):
  hw_details= config_obj.hw_details
  
  if config_obj.run_mode == config_obj.run_mode_enum.FULL:
    list_of_partitions, status_dict= global_barriers(net, graph, graph_nx, node_w, config_obj)
    if config_obj.write_files:
      with open(config_obj.get_partitions_file_name(), 'wb+') as fp:
        logging.warning(f"Writing partitions to files at {config_obj.get_partitions_file_name()}")
        pickle.dump((list_of_partitions, status_dict), fp)

  elif config_obj.run_mode == config_obj.run_mode_enum.RESUME:
  ...
  # NOTE: to correct for a bug
  # TODO: must remove following 2 for loops
  # offset= min(list(status_dict.keys()))
  # for n_correct, _ in list(graph.items()):
  #   n = n_correct + offset
  #   status_dict[n_correct] = status_dict[n]
  #   del status_dict[n]
  # for pe_id, pe_schedules in enumerate(list_of_partitions):
  #   for global_barrier_idx, pe_schedule in enumerate(pe_schedules):
  #     pe_schedule_corrected= []
  #     for n in pe_schedule:
  #       n_correct= n - offset
  #       pe_schedule_corrected.append(n_correct)
  #     list_of_partitions[pe_id][global_barrier_idx]= pe_schedule_corrected

  # pe_id assert after partitioning
  for node, obj in list(graph.items()):
    if not obj.is_leaf():
      assert status_dict[node].pe_id != None

  
  # list of N_PE lists
  # Each list further consists of multiple sublists. Each sublist coreesponds to instructions to be performed in-between global barriers
  # Local barriers are inserted as instructions within this sublists
  # Basic object of these sublists is of type instr_types.full_instr
  list_of_schedules= local_barriers.main(graph, graph_nx, list_of_partitions, status_dict, hw_details, final_output_nodes)

  memory_allocation.main(graph, status_dict, list_of_schedules, hw_details, final_output_nodes)
  
  list_of_schedules= insert_set_ld_stream_len_instr(list_of_schedules)
  
  if verbose:
    print_statistics(graph, list_of_schedules, status_dict)

  return status_dict, list_of_schedules, hw_details, list_of_partitions
...
def global_barriers(net, graph, graph_nx, node_w, config_obj):
  ...
  if mode == config_obj.partition_mode_enum.HEURISTIC:
    ...
    # layer sets to help reduce graph size during two_way_partition
