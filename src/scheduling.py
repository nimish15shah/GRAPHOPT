  ...
  while not graph[head_node].computed:
    ...
    if write_asm:
      if curr_bb in bb_w_leaf_inputs:
        reporting_tools.reporting_tools.write_asm(global_var, 'ld_v', ['mem_x', str(curr_bb)])
    ...
    if write_asm:
      for pos in pos_lst:
        reporting_tools.reporting_tools.write_asm(global_var, 'cp_StV', [str(pos), str(curr_bb)])
    
    # Free up Scalar file
    scalar_reg_file.deallocate_AppIdx(curr_bb)
    
    if write_asm:
      reporting_tools.reporting_tools.write_asm(global_var, 'BB', [str(curr_bb)])
...
def allocate_output(graph, BB_graph, active_bb, score_inputs, curr_bb, head_node, vect_reg_file, scalar_reg_file, global_var, write_asm):
  ...
  for par, out_list in list(map_par_to_out.items()):
    if par in par_in_active_bb:
      ...
      #if write_asm:
      #  for pos in pos_lst:
      #    reporting_tools.reporting_tools.write_asm(global_var, 'cp_StV', ['pos:' + str(pos), 'par:' + str(par)])
      ...
      for out in out_list:
        if out_scalar_status[out] == False:
          ...
          if write_asm:
            reporting_tools.reporting_tools.write_asm(global_var, 'push_out_to_S', [str(curr_bb), 'pos:' + str(scalar_pos), 'par:', str(par) ])
  ...
  #  if write_asm:
  #    reporting_tools.reporting_tools.write_asm(global_var, 'secondary_push', [str(curr_bb)])
  ...
  if write_asm:
    for idx, chunks in enumerate(combined_out_list_chunks):
      reporting_tools.reporting_tools.write_asm(global_var, 'BB_' + str(idx), [curr_bb] + [chunks])
    
    if not combined_out_list:
      reporting_tools.reporting_tools.write_asm(global_var, 'BB_U', [curr_bb])
...
def load_scalar_to_vect_basic_op(BB_graph, global_var, pos, curr_batch_of_BB, scalar_reg_file, active_bb, vect_reg_file, write_asm):
  # Perform actual copy
  if write_asm:
    reporting_tools.reporting_tools.write_asm(global_var, 'cp_StV_vect', ['pos:' + str(pos), 'par_lst:' + str(curr_batch_of_BB)])
...
def load_leaf(graph, BB_graph, curr_bb, track_leaf_status, track_leaf_pos, scalar_reg_file, global_var, write_asm):
  ...
  if len(curr_bb_obj.leaf_inputs) <= VECT_LOAD_THRESHOLD:
      ...
      if track_leaf_status[leaf] == False: # Leaf not loaded to scalar mem
        ...
        if not curr_bb == sort_par[0]: # Need to load to scalar as there are multiple parents left
          ...
          if not LOAD_DIRECT:
            ...
            if write_asm:
              reporting_tools.reporting_tools.write_asm(global_var, 'ld', [str(leaf), str(pos)])
          ...
          # Load directly to the vector regfile
            ...
            if write_asm:
              reporting_tools.reporting_tools.write_asm(global_var, 'ldw_v', [str(leaf), str(curr_bb)])
    ...
    if len(only_this_par_lst) > VECT_LOAD_THRESHOLD:
      if write_asm:
        reporting_tools.reporting_tools.write_asm(global_var, 'ldv_v', [str(only_this_par_lst), str(curr_bb)])
    else:
...
def BB_scheduling(graph, BB_graph, head_node, global_var, leaf_list, write_asm= False, make_vid= False):
  ...
  if make_vid:
    st= time.time() 
    vid_maker= reporting_tools.reporting_tools.video_maker(global_var.VID_FILE , global_var.IMG_FILE_FOR_PYDOT)
