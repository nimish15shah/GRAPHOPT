...
def instruction_gen(net, graph, BB_graph, global_var, leaf_list, misc, hw_details, w_conflict, MEM_LOAD_CONST, base_scratch_pad_addr, last_scratch_pad_addr, base_param_addr, last_param_addr, SCHEDULING_SEARCH_WINDOW, RANDOM_BANK_ALLOCATE, write_asm= False, make_vid= False, verbose= False):
  
  printlog('ALERT!!! SCHEDULING_SEARCH_WINDOW: ' + str(SCHEDULING_SEARCH_WINDOW), 'red')
  ...
  # Add initial loads
  initial_ld_cnt, map_param_to_addr= add_initial_loads(graph, BB_graph, instr_ls_obj, n_mem_banks, leaf_list, base_param_addr, last_param_addr)
...
#  liveness_analysis(graph, BB_graph, IO_graph, bb_seq, bank_d, num_of_banks, global_var)
  ...
  instr_ls_obj.instr_ls= pipeline.reschedule_for_hazardfree(instr_ls_obj.instr_ls, n_pipe_stages, n_mem_banks, SCHEDULING_SEARCH_WINDOW)
  ...
  extra_load_stores(graph, BB_graph, instr_ls_obj, num_of_banks, reg_bank_size, base_scratch_pad_addr, last_scratch_pad_addr, n_pipe_stages, verb= verbose)  

  pipeline.sanity_check_hazard(instr_ls_obj.instr_ls, num_of_banks, n_pipe_stages)
  ...
  print_instr_breakup(instr_ls_obj, conflicts_in, conflicts_out, conflicts_ld)
  
  return instr_ls_obj, map_param_to_addr
...
def print_instr_breakup(instr_ls_obj, conflicts_in, conflicts_out, conflicts_ld):
  """
    Display instruction breakup
  """
  print(" ")
      ...
      if CONFLICT:
        ...
        for bank, node_ls in list(bank_to_node_d.items()):
          for node_idx, node in enumerate(node_ls):
            ...
            # This is because conflicting_nodes function has a problem in bb_out mapping
...
def extra_load_stores(graph, BB_graph, instr_ls_obj, num_of_banks, reg_bank_size, base_scratch_pad_addr, last_scratch_pad_addr, n_pipe_stages, verb= False):
  """
  ...
  while curr_instr_idx < len(instr_ls_obj.instr_ls):
    ...
    add_store_load_if_needed(curr_instr_idx, consumed_pos, free_pos, instr_ls_obj, node_details_dict, reg_status, reg_bank_size, map_bb_to_cycle, map_bb_to_instr_id, free_mem_addr, n_pipe_stages)
...
def add_store_load_if_needed(curr_instr_idx, consumed_pos, free_pos, instr_ls_obj, node_details_dict, reg_status, reg_bank_size, map_bb_to_cycle, map_bb_to_instr_id, free_mem_addr, n_pipe_stages):
  ...
  if FULL:
    ...
    for bank,pos_set in enumerate(consumed_pos):
      if len(pos_set) > FULL_THRESHOLD or (reg_bank_size-len(pos_set))< FULL_WATERMARK: 
        ...
        map_nodes_to_parent_cycle_temp= map_node_to_first_parent_cycle(curr_instr_idx, node_lst, bank ,instr_ls_obj.instr_ls)
    ...
    assert len(free_mem_addr) != 0, "Scratch pad size not enough"
    ...
    earliest_parent= min(store_nodes_set, key= lambda x: map_nodes_to_parent_cycle[x])  
    earliest_parent_cycle= map_nodes_to_parent_cycle[earliest_parent]
    ld_cycle= earliest_parent_cycle + 1 - n_pipe_stages
    
    assert ld_cycle > st_cycle + 1, [ld_cycle, st_cycle, store_nodes_set]
...
def map_node_to_first_parent_cycle(curr_instr_idx, node_lst, bank, instr_ls):
...
def add_initial_loads(graph, BB_graph, instr_ls_obj, n_mem_banks, leaf_list, base_addr, last_addr):
  """
    Group together leaf inputs and add vector loads in instruction sequence
  """
  LOAD_LEN= n_mem_banks - 4
  ...
  for load_idx, load_details in enumerate(load_list_details):
    ...
    assert instr.mem_addr <= last_addr
...
def liveness_analysis(graph, BB_graph, IO_graph, bb_seq, bank_d, total_banks, global_var):
  ...
  for bank, profile in list(bank_occup_profile.items()):
    reporting_tools.reporting_tools._write_csv(global_var.BANK_OCCUP_PROFILE_FILE, profile, mode= 'a')
