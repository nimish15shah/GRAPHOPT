...
def plot_superlayers_ops_vs_layers(name_ls, n_threads_ls, plot_d):
  for name in name_ls:
    list_of_partitions_2, node_w, config_obj, graph_nx, graph = plot_d[(name,2)]

    ops_in_super_layer_2= [0] * len(list_of_partitions_2[0])
    for pe in list_of_partitions_2:
      for l, part in enumerate(pe):
        ops_in_super_layer_2[l] += sum([node_w[n] for n in part])

    status_dict_dummy= partition.init_status_dict(graph)
    _, layer_sets= partition.layer_wise_partition_ALAP(graph_nx, status_dict_dummy, config_obj)

    y_dag_ops= [sum([node_w[n] for n in l]) for l in layer_sets]
    y_2= ops_in_super_layer_2
    bar_width= 0.1

    fig_dims = (2, 2)
    fig, ax = plt.subplots(figsize=fig_dims) 

    # ax.plot(x, y_dag_ops, 'o')

    x= list(range(1, len(y_2)+1))
    # ax.bar(x, y_2, width= bar_width*np.array(x))
    ax.plot(x, y_2, 'o')
    
    N= 5
    y_2_smooth= y_2[:N-1] + list(running_mean(y_2, N))
    # ax.plot(x, y_2_smooth)

    x= list(range(1, len(y_dag_ops)+1))
    # ax.bar(x, y_dag_ops, width= bar_width*np.array(x))
    ax.plot(x, y_dag_ops, 'o')

    N= 5
    y_dag_ops_smooth= y_dag_ops[:N-1] + list(running_mean(y_dag_ops, N))
    # ax.plot(x, y_dag_ops_smooth)

    ax.set_yscale('log')
    ax.set_xscale('log')

    xticks= [10**(a) for a in range(0, 1+ int(math.log10(len(y_dag_ops))))]
    print(xticks)
    plt.xticks(xticks)
    ...
    # path= './no_backup/' + name.replace('/', '_') + '_super_layer_ops.png'
