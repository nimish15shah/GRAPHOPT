...
from .optimization import write_to_file
from .optimization import pe_bank_allocate
...
#  pe_bank_allocate.statistics_of_IO_graph(graph_nx, IO_graph)
#  pe_bank_allocate.graph_color_clique(net, graph_nx, BB_graph, IO_graph)
#  exit(1)
#  fpath= '/users/micas/nshah/Downloads/no_backup/Setups/minizinc_code/no_backup/pe_bank_allocate.dzn'
...
#  exit(1)
  ...
  conflicts, conflicts_in, conflicts_out, conflicts_ld= count_conflicts(BB_graph, IO_graph, instr_ls_obj, new_color_d, n_mem_banks)
...
#  conflicts= count_conflicts(BB_graph, IO_graph, instr_ls_obj, new_color_d, n_mem_banks)
  ...
  conflicts, conflicts_in, conflicts_out_0, conflicts_ld= count_conflicts(BB_graph, IO_graph, instr_ls_obj, new_color_d, n_mem_banks)
  assert conflicts_out_0 <= conflicts_out
...
#      curr_node= min(candidate_list, key= lambda x: len(node_attr_d[x].allowed_banks) + WT_DEGREE_1 * IO_graph.degree(x))
    ...
    if len(node_obj.allowed_banks): # Color available
      ...
      # TODO: Choose the one with least conflicts
...
def count_conflicts(BB_graph, IO_graph, instr_ls_obj, color_d, total_colors):
  """
